...
public class ExecutorServiceThreadPoolTest {
	...
	/**
	 * Configura o quartz com o {@link ExecutorServiceThreadPool}
	 *
	...
	private void triggerJob(Class<? extends Job> classe)
		...
		Trigger trigger = TriggerBuilder.newTrigger()
				...
				.withSchedule(SimpleScheduleBuilder.simpleSchedule().withIntervalInMilliseconds(200L).withRepeatCount(10))
				.build();
	...
	public void increaseThreadPool() throws SchedulerException {
		...
		while (!scheduler.getTriggerGroupNames().isEmpty()) {
			...
			if (volta == 2) {
				DynamicThreadPool threadPool = DynamicThreadPoolRepository.getInstance().lookup(QUARTZ_SCHEDULER);
				threadPool.doResize(INCREASE_POOL_SIZE);
			}
